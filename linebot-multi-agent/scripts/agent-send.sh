...
# Agent Communication Script for LINEBot Multi-Agent System
...
usage() {
    ...
    echo ""
    echo "Options:"
    echo "  -f FROM_AGENT   The sending agent (ARCHITECT, BACKEND, FRONTEND, TESTER)"
    echo "  -t TO_AGENT     The receiving agent (ARCHITECT, BACKEND, FRONTEND, TESTER, ALL)"
    echo "  -m MESSAGE      The message to send"
    echo "  -p PRIORITY     Message priority (HIGH, MEDIUM, LOW) [default: MEDIUM]"
    echo ""
    echo "Examples:"
    echo "  $0 -f ARCHITECT -t BACKEND -m 'Please implement the webhook endpoint'"
    echo "  $0 -f BACKEND -t ALL -m 'Webhook implementation complete' -p HIGH"
    exit 1
}
...
# Validate agents
VALID_AGENTS="ARCHITECT BACKEND FRONTEND TESTER"
