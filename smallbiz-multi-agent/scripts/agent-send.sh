...
# Agent Communication Script for Small Business Planning Multi-Agent System
...
usage() {
    ...
    echo ""
    echo "Options:"
    echo "  -f FROM_AGENT   The sending agent (STRATEGIST, RESEARCHER, MARKETER, ENGINEER, FINANCIER)"
    echo "  -t TO_AGENT     The receiving agent (STRATEGIST, RESEARCHER, MARKETER, ENGINEER, FINANCIER, ALL)"
    echo "  -m MESSAGE      The message to send"
    echo "  -p PRIORITY     Message priority (HIGH, MEDIUM, LOW) [default: MEDIUM]"
    echo ""
    echo "Examples:"
    echo "  $0 -f STRATEGIST -t RESEARCHER -m 'Please analyze the target market'"
    echo "  $0 -f RESEARCHER -t ALL -m 'Market analysis complete' -p HIGH"
    exit 1
}
...
# Validate agents
VALID_AGENTS="STRATEGIST RESEARCHER MARKETER ENGINEER FINANCIER"
